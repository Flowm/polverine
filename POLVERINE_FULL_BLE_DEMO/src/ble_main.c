 ...
 */
 ...
 char SAMPLE_DEVICE_NAME[20] = {0};  //The Device Name Characteristics in GAP
 ...
 static uint8_t spp_adv_data[25] = {
     ...
     0x11,0x09, 'P', 'O', 'L', 'V', 'E', 'R', 'I', 'N', 'E', '_', '0', '0', '0', '0', '0', '0'
 };
...
void ble_send_message(const char *message)
{
    if (is_connected && enable_data_ntf) {
        esp_ble_gatts_send_indicate(spp_gatts_if, spp_conn_id, spp_handle_table[SPP_IDX_SPP_DATA_NTY_VAL], strlen(message), (uint8_t *)message, false);
    }
}   
...
extern char shortId[7];
 ...
 void ble_init(void)
 {
    snprintf(SAMPLE_DEVICE_NAME,20,"POLVERINE_%s",shortId);

    for (int i = 0; i < 6; i++) {
        spp_adv_data[19+i] = shortId[i];
    }
    esp_err_t ret;
