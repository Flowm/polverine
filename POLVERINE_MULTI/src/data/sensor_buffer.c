...
void bme690_buffer_add(bme690_sensor_buffer_t *buffer, const bme690_data_t *data)
{
    ...
    ESP_LOGD(TAG, "Added BME690 sample #%lu (buffer: %d/%d)", 
             (unsigned long)buffer->total_samples, buffer->sample_count, MAX_SENSOR_SAMPLES);
}
...
void bmv080_buffer_add(bmv080_sensor_buffer_t *buffer, const bmv080_data_t *data)
{
    ...
    ESP_LOGD(TAG, "Added BMV080 sample #%lu (buffer: %d/%d)", 
             (unsigned long)buffer->total_samples, buffer->sample_count, MAX_SENSOR_SAMPLES);
}
